...
To use LoRA features (training or local inference):
```bash
pip install .[lora]
# or
...
| `MemoryManagerConfig`           | `openai`, `deepseek`, `local_hf_lora` |
...
| `memory_manager`      | `MemoryManagerConfig()`                     | dict / object. Controls the model used to generate summaries and metadata (`MemoryManagerConfig`). Options: `openai`, `deepseek`, `local_hf_lora`. For `local_hf_lora`, you can specify `adapter_path`, `device_map`, and `torch_dtype`. |
